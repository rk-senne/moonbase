...
### Added
- docs(agents): adopt four SwarmForge agent-discipline guardrails into agent doctrine and steering — (1) **anti-proxy verification** (never substitute a homegrown script for a real build/test/lint/coverage/vuln tool; a script that prints `PASS` is not a gate), (2) **scratch-file discipline** (temporary files live in `./tmp/` inside the repo, never system `/tmp`), (3) **commit attribution** (every operative commit carries an `Operative: numbuh-<N> (<Designation>)` byline trailer; hooks are never bypassed with `--no-verify`), and (4) **fail-closed on unexpected layout** (stop and report when the git layout or a required directory is missing rather than working in the wrong place). The full canon lives once in `.kiro/steering/production-standards.md` (§ Agent Discipline Guardrails) with reinforcement in `quality-gates.md`; each of the 14 agents carries a role-scoped `## Operative Discipline` pointer — write/shell operatives (numbuh-3, 9, 362, 999, knd-council) get all four, read-only and analysis operatives get anti-proxy + fail-closed only.
- feat(discovery): file-level **Code Dependency Graph** (`internal/discovery/depgraph.go`). Exposes `DirectDependencies`, `RelatedFiles` (transitive closure both directions, for retrieving only the files relevant to a change instead of the whole codebase) and `AffectedFiles` (reverse closure — "if I change this, what breaks?"). Forward and reverse adjacency behind an `RWMutex`, cycle-safe BFS, deterministic sorted output. Purely additive: `ProjectContext.DepGraph` is nil when unavailable and `HasDepGraph()` guards every use, so existing discovery behaviour is unchanged. Built by the moonbase agents themselves; measured 279 nodes on this repo, with 78 files transitively affected by `internal/agents/registry.go`.
- feat(discovery): file-level **code dependency graph** for Go projects. `Discover` now builds an import graph (stdlib `go/parser`, no new dependency) exposed as `ProjectContext.DepGraph`, letting agents retrieve only the files related to a change (`RelatedFiles`), find what a change can affect (`AffectedFiles`), or list direct imports (`DirectDependencies`) instead of scanning the whole codebase. Fully additive and cycle-safe: the graph is nil for non-Go projects, missing `go.mod`, or build failure, so all existing discovery behaviour is unchanged when the graph is unavailable.
...
- feat(backend): `KIMI_BASE_URL` overrides the Kimi API endpoint, mirroring the existing `OPENAI_BASE_URL`. Removes a hardcoded endpoint and supports proxies or gateways in front of Moonshot.
- docs(spec): `.kiro/specs/tui-decomposition/design.md` records the staged plan for breaking up `internal/tui`, including the circular-dependency constraint (all 25 view functions are methods on `App`) that blocks a naive package split.

- feat(agents): every operative now carries a **Reference Knowledge** section — role-scoped engineering doctrine extracted from all 13 books in the research corpus (`research/`) and attributed to its source. All 14 agents updated, each mapped to the books relevant to its role: requirements, estimation and SLO definition (Numbuh 1); component coupling, boundary cost and data-model trade-offs (Numbuh 2, Numbuh 0); implementation and language-level correctness (Numbuh 3); verification technique and percentile discipline (Numbuh 4); review lenses (Numbuh 5); trust boundaries and defensive Go (Numbuh 274); flow, constraints and release safety (Numbuh 362); migration and schema-evolution patterns (Numbuh 9); failure modes and distributed-systems hazards (Numbuh 13); decommissioning signals and debt economics (Numbuh 86); diagram, comment and operability documentation rules (Numbuh 999); legacy coupling archaeology (Sector Z); and pipeline orchestration physics (KND Council).
- feat(skills): six new curated skills — `architecture-boundaries`, `architecture-diagrams`, `design-patterns`, `incremental-migration`, `distributed-data`, and `algorithmic-complexity` — covering domains the library previously had no skill for (architecture structure, C4 diagramming, pattern selection, evolutionary migration, cross-boundary data ownership, complexity analysis). Library grows from 10 to 16 skills; each new skill body is kept under the 2,000-rune progressive-load limit so it loads without truncation.
- docs(doctrine): `ArchitectureDoctrine.md` gains measurable **Component Structure** rules (REP/CCP/CRP cohesion tension, ADP/SDP/SAP, the Main Sequence and its `D = |A + I − 1|` metric, boundary-cost ladder, compiler-enforced boundaries), a **Duplication** section separating true from accidental duplication, and a **Reference Canon** table tracing each rule to its source.
- feat(tui): every dev tool now carries a **"why it's useful"** note — shown as a `💡` detail line for the focused tool in the Settings and Tools views, alongside its one-line description.
...
### Fixed
- fix(config): `phase_timeout_seconds`, `max_output_size` and `max_retries` were documented and settable but **inert** — nothing copied them onto the pipeline, so `pipeline.New`'s hardcoded defaults always won. Raising `phase_timeout_seconds` had no effect, and `--trace` printed the hardcoded value, which made it look as though the setting was honoured. One `applyPipelineConfig` helper now applies config at all three construction sites; `--fast` previously ignored config entirely, dropping even `parallel_specialists`. Zero values are ignored so an unset key keeps the default rather than clamping to zero.
- fix(pipeline): default phase timeout raised 5m → 20m. The old value was harmless while timeouts were unenforceable, but a measured implementation phase takes 5m43s, so enforcing 5m would have caused spurious cancellations.
- fix(security): CI was red on `govulncheck` and is now green — 5 vulnerabilities to 0. Go bumped 1.26.5 → 1.26.6 for four standard-library fixes (GO-2026-6218 quadratic `net/url` `resolvePath`, GO-2026-6090 `crypto/tls` post-handshake limit, GO-2026-6088 `encoding/xml` recursion depth, GO-2026-5972 `encoding/asn1` recursion depth), and `golang.org/x/net` 0.39.0 → 0.55.0 for GO-2026-5026 (Punycode label rejection), which needed both. This failure pre-dated the current work: the last green CI run was the dev-tools note commit, and it broke on the subsequent dependabot bumps.
- fix(backend): **phase timeouts now actually cancel work.** Neither `Backend.Deploy` nor `RawDeployer.DeployRaw` accepted a context, so `DeployComposed` created a deadline the non-streaming path could not observe: `exec.Command` was uncancellable and the call blocked until kiro-cli exited on its own. A 5-minute phase timeout was measured taking **11 minutes**, and combined with auto-retry a single stuck phase could hold the pipeline for roughly 45 minutes. Adds the optional `RawContextDeployer` interface (preferred by `DeployComposed`), switches Kiro to `exec.CommandContext`, and bounds the wait with `WaitDelay` — because killing kiro-cli alone does not release stdout, which its descendants (`kiro-cli-chat`, `bun`) keep open. Cancellation now returns in ~5s instead of hanging.
- fix(backend): the streaming path had the same hang. Its goroutine blocked in `Scan()` on a pipe held open by descendants, so `ctx.Done()` was never reached and `WaitDelay` could not help (nothing had called `Wait` yet). A watchdog now closes the pipe on cancellation, releasing the scanner.
- refactor(backend): Kiro extracted from `backends.go` into `kiro.go` (328 → 200 lines), matching the existing per-backend file pattern and dropping the file-size baseline from 17 entries to 16.
...
- refactor(cmd): `mission.go` split from **796 lines to 227** across seven cohesive files by reason-to-change — entry points, the pipeline loop, single-phase execution, specialist fan-out, gates, context injection, and token budgeting. It no longer appears in the file-size baseline (18 → 17 entries). The extracted `phaseBudget` gate is now unit-testable in isolation rather than only reachable through a full mission run.
- fix(discovery): curated skills are no longer silently truncated when loaded. Every one of the 10 original skills exceeded the 2,000-rune budget, so their closing sections were dropped on load. The budget is now split by purpose — legacy skills injected into every prompt stay bounded at 2,000 runes, while a skill the agent explicitly requests via `@skill(name)` gets 8,000.
- fix(discovery): prompt-content truncation no longer slices by bytes, which could cut a multi-byte UTF-8 character in half and produce a string that `kiro-cli` rejects as invalid UTF-8. All three truncation sites now share one rune-safe helper that also cuts at a line boundary rather than mid-sentence.
- fix(backend): a backend with an exported-but-empty API key no longer reports itself as available. `MOONSHOT_API_KEY=` (or the OpenAI/Anthropic equivalents) previously made auto-detection select a backend that then failed every deploy with "not set".
- test(updater): coverage raised from 31.1% to 76.4% on the code that replaces the running binary, including the checksum-verification path (a tampered artifact is now proven to be rejected) and the install rollback path (the original binary is proven to be restored when installation fails).
- test(backend): coverage raised from 64.9% to 72.1%, covering the previously untested Kimi backend and OpenAI's `DeployWithUsage` token-accounting path.
- refactor(fuzzy): the fuzzy matcher moved out of `internal/tui` into `internal/fuzzy` — a pure scoring algorithm with no presentation concern, now at 100% coverage.
- refactor(updater): split by reason-to-change into `updater.go` (decide and install) and `release.go` (talk to GitHub).
- fix(research): repaired the research knowledge index — the corpus is now **13 books, all indexed and verified searchable**, up from 4. Previously 3 of the 8 books on disk (*Monolith to Microservices*, *Head First Design Patterns*, *Learning Java*) were never ingested and returned zero search results, and a further 5 books (*The Pragmatic Programmer*, *A Philosophy of Software Design*, *The Phoenix Project*, *Designing Data-Intensive Applications*, *Black Hat Go*) had been deleted from the working tree entirely — so agent guidance attributed to any of them was unverifiable. The deleted PDFs were restored from git (the deletions were unstaged) and every book indexed.
- fix(research): worked around two indexer failure modes discovered while re-indexing. PDFs with a malformed font resource (`font resource is not a dictionary`) fail text extraction silently — the context registers but contains no searchable chunks. Others register with `Items: 1` yet every search returns `Context not found`, whether indexed concurrently or strictly one at a time. Affected books index reliably from `pdftotext` output; `scripts/extract-research-text.sh` regenerates it (output is gitignored — the PDFs remain the source of truth).
- fix(agents): audited every citation added to the new Reference Knowledge sections so none attribute guidance to a book that isn't present and searchable in the corpus.
...
### Changed
- refactor(agents): reframed test discipline from human ceremony to agent values. Numbuh 3's three-laws TDD section is replaced by a **Build Loop** whose unit is one independently verifiable increment (a function, a branch, an error path) rather than a whole acceptance criterion, plus a **Green Ledger** recording increment → expectation → test → status so "is this green?" is a checkable artifact instead of an assertion. The stopwatch cycles and "no more test than sufficient to fail" are gone; what stays — and is stated as a value, not a ritual — is that the expectation must exist before the implementation, because an agent shown code writes tests that agree with it, bugs included.
- refactor(agents): Numbuh 4 now audits the Green Ledger row by row and hunts tests that ratify the implementation rather than verify the requirement (expected values copied from actual output, surprising behaviour documented as intended, tests meaningless without the implementation). He can also accept increments as they land instead of waiting for a whole phase. Handover discipline is now "every claim carries its evidence" — the collaboration rule that replaces trust, since agents can attach proof where humans must rely on it.
