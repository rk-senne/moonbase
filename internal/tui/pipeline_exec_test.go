...
func TestPhaseResultMsg_Success(t *testing.T) {
	...
	if result.pipelineState.Current != 1 {
		t.Errorf("expected pipeline to advance to phase index 1, got %d", result.pipelineState.Current)
	}
	if result.pipelineState.Phases[0].Status != pipeline.StatusComplete {
		t.Errorf("expected phase 0 to be complete, got %d", result.pipelineState.Phases[0].Status)
	}
	// Output should be recorded in context
	if result.pipelineState.Context.PhaseOutputs[1] == "" {
...
func TestPhaseResultMsg_Error(t *testing.T) {
	...
	if result.pipelineState.Current != 0 {
		t.Errorf("expected pipeline to stay at phase 0, got %d", result.pipelineState.Current)
	}
	// Current phase should be marked as failed
	if result.pipelineState.Phases[0].Status != pipeline.StatusFailed {
		t.Errorf("expected phase 0 to be failed, got %d", result.pipelineState.Phases[0].Status)
	}
	...
	found := false
	for _, msg := range result.pipelineChat {
...
func TestPhaseResultMsg_RiskGate_Low(t *testing.T) {
	...
	if result.pipelineState.Context.RiskLevel != "LOW" {
		t.Errorf("expected risk level LOW, got %s", result.pipelineState.Context.RiskLevel)
	}
	// Should have advanced (current >= 4 since phase 4 index is 3)
	if result.pipelineState.Current < 4 {
		t.Errorf("expected pipeline to advance past QA (index 3), got current=%d", result.pipelineState.Current)
	}
}
...
func TestPhaseResultMsg_RiskGate_Medium(t *testing.T) {
	...
	if result.pipelineState.Context.RiskLevel != "MEDIUM" {
		t.Errorf("expected risk level MEDIUM, got %s", result.pipelineState.Context.RiskLevel)
	}
	// MEDIUM should route back to implementation (phase 3, index 2)
	if result.pipelineState.Current != 2 {
		t.Errorf("expected pipeline to route back to phase index 2 (implementation), got %d", result.pipelineState.Current)
	}
}
...
func TestPhaseResultMsg_RiskGate_Critical(t *testing.T) {
	...
	if result.pipelineState.Context.RiskLevel != "CRITICAL" {
		t.Errorf("expected risk level CRITICAL, got %s", result.pipelineState.Context.RiskLevel)
	}
...
func TestStartNextPhase_ConditionalSkip(t *testing.T) {
	...
	app.pipelineState.Phases[5].Status = pipeline.StatusRunning
