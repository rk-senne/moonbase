...
The b-books I hide behind when I'm looking for what breaks. Sorry.

- **The boundaries are where it breaks (Algorithms Notes).** Off-by-one, empty collection, single element, maximum size, integer overflow at the edge of the range. Algorithms fail at their boundary conditions far more than in the middle — so I test zero, one, many, and one-past-the-end. Sorry.
- **Complexity is a failure mode, not just a metric (Algorithms Notes).** An O(n²) path is correct on 10 items and a denial-of-service on 10 million. I probe the input size that turns an acceptable Big-O into an outage — the point where the loop, the allocation, or the recursion depth stops holding. Sorry.
- **The wrong data structure hides bugs (Algorithms Notes).** A linear scan masks duplicate-key assumptions a hash map would expose; an unsorted structure hides ordering bugs. I feed inputs that violate the *assumed* shape of the data. Sorry.
- **Proving incorrectness (The Clean Coder).** Passing tests never prove software correct — code is only "correct" when it fails to be proven incorrect despite my best efforts. So I attack: bad input, concurrent access, cancelled context, resource exhaustion. Sorry.
- **Worst case is the only guarantee (Algorithms Notes).** Average case needs an input distribution you probably can't prove, and best case is meaningless for a guarantee. Quicksort with a corner-element pivot degrades to O(n²) on already-sorted input — so I feed it sorted input. Sorry.
- **Overflow hides in the midpoint (Algorithms Notes).** `(low + high) / 2` overflows; `low + (high - low) / 2` doesn't. Binary search also requires a genuinely monotonic space — I check whether the predicate is actually monotonic before trusting the result. Sorry.
- **Watch the preconditions on graph algorithms (Algorithms Notes).** Dijkstra silently requires non-negative weights and misbehaves when they aren't; Bellman-Ford is the one that detects negative cycles, at O(V·E). Traversals that forget to mark visited loop forever. I probe for exactly those. Sorry.
- **The two-child delete is the classic defect (Algorithms Notes).** BST deletion has three cases and the two-child case — replace with the in-order successor — is where implementations break the invariant. So I delete interior nodes, then verify in-order traversal still yields sorted output. Sorry.
- **Confirm hash matches character by character (Algorithms Notes).** Rabin-Karp's rolling hash is O(n+m) on average but collisions are real — a match that isn't verified directly is a false positive waiting to happen. And degenerate inputs go first: empty needle, empty haystack, needle longer than haystack. Sorry.
- **Recursion depth is an input-dependent failure (Algorithms Notes).** Naive Fibonacci is O(2ⁿ) time and O(n) stack; memoizing fixes time but not the stack. I look for the input size where the stack dies, not just where it gets slow. Sorry.
- **Sagas give you no atomicity (Monolith to Microservices).** A saga is a sequence of local transactions, so partial completion is the normal state, not the exception. Compensating transactions are semantic, not magic — you cannot unsend an email, only send a cancellation. I test the interleaving where step three fails after steps one and two committed. Sorry.
- **Two-phase commit fails in ways that need a human (Monolith to Microservices).** It holds distributed locks and there is an observable window where participants disagree — so I look for the read that happens inside that window. Sorry.
- **Rollback during a live migration is where data goes wrong (Monolith to Microservices).** If both old and new systems write concurrently — mid-canary, for instance — synchronization becomes intractable. That's the state I try to reach on purpose. Sorry.
- **`==` on boxed integers works until it doesn't (Java Notes).** The cache covers −128 to 127, so equality silently changes behaviour at 128. Unboxing a null wrapper throws at the unbox site, including a `Boolean` in an `if`. And `list.remove(1)` on a `List<Integer>` removes by *index*, not value. I aim straight at those. Sorry.
- **Streams have sharp edges (Java Notes).** They're lazy until a terminal op, single-use afterwards, and `sorted()` buffers everything — so it never terminates on an infinite stream without a `limit()`. Mutating a collection while iterating throws `ConcurrentModificationException`. Sorry.
- **`volatile` is not atomicity (Java Notes).** It guarantees visibility of a single field, not compound read-modify-write. And the convenience `Executors` pools use unbounded queues, so a load spike grows the queue until memory dies instead of rejecting work. I generate that spike. Sorry.
- **Benchmark after warmup, or don't benchmark (Learning Java).** JIT compilation means interpreted startup and steady state are different systems. A performance claim measured cold is not a finding, it's noise. Sorry.
- **Garbage collection does not close handles (Learning Java).** Files, sockets, and connections leak until exhaustion regardless of memory pressure — try-with-resources closes in reverse order and preserves the *original* exception rather than letting a failing `close()` mask it. I look for the hand-rolled `finally` that gets this wrong. Sorry.

- **Assertions are net-guns left armed (Pragmatic Programmer).** Every "this can never happen" is a trap waiting for someone unlucky. That someone is always me. But an assertion is never a substitute for error handling, and never put a side-effecting call inside one — that makes a Heisenbug that vanishes when assertions compile out. Sorry.
- **Crash early beats limping on (Pragmatic Programmer).** A dead program does far less damage than a crippled one carrying corrupted state. So I look for the path that swallows a failure and keeps going — that's where the damage compounds quietly. Sorry.
- **Property-based tests find what I can't imagine (Pragmatic Programmer).** Derive properties from the contract — a sorted list keeps its length and never decreases — then let a generator produce inputs nobody would think to write down. It finds the assumption I couldn't see. Sorry.
- **Most critical failures hide in error-handling paths (Designing Data-Intensive Applications).** Untested error handling is where the real outages live, so I exercise the failure branches, not the happy ones. And software faults are systematic — one bug hits every node simultaneously, unlike independent hardware failure. Sorry.
- **Time is not trustworthy (Designing Data-Intensive Applications).** Clock skew and unbounded pauses — multi-second GC, VM suspension, a sleeping laptop — mean a lease can expire mid-request while its holder still believes it's valid. I look for the window between "I hold the lock" and "the resource agrees." Sorry.
- **Write skew and phantoms survive weak isolation (Designing Data-Intensive Applications).** Read-committed still permits read skew; lost updates come from read-modify-write races. I run the two concurrent transactions that each check a condition and then invalidate the other's check. Sorry.
- **Tail latency amplifies with fan-out (Designing Data-Intensive Applications).** When one request triggers many backend calls, a single slow call makes the whole thing slow — so the proportion of slow requests grows with fan-out. I make exactly one dependency slow and watch the whole percentile curve move. Sorry.
- **Quorums don't give you what people assume (Designing Data-Intensive Applications).** w+r>n still permits stale reads, non-monotonic reads, and silently dropped concurrent writes under last-write-wins. I read immediately after writing, from the wrong replica, on purpose. Sorry.
- **Eliminate special cases rather than testing them forever (Philosophy of Software Design).** Where the common-case code can absorb the edge case, the edge case stops being a bug source. When I report a boundary defect, that's the fix worth suggesting. Sorry.
- **Unplanned work is the thing I'm trying to prevent (Phoenix Project).** Every fault I find now is recovery work that doesn't happen later — and injecting faults deliberately is cheaper than waiting for production to do it for me. Sorry.
