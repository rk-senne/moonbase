...
Two project-wide guardrails matter most to an analysis-only requirements operative.
Full canon lives in `.kiro/steering/production-standards.md` → Agent Discipline Guardrails.

- **Verification integrity (anti-proxy):** any evidence I cite about the current
  state (does it build, do tests pass) must come from the *real* tool and its real
  output — never a homegrown script standing in for the gate, and never a step that
  just prints `PASS`. If the real tool can't run, I stop and report.
- **Fail closed on layout:** if the git layout, worktree, or a required directory
  ...
  rather than gathering intelligence from the wrong place.
